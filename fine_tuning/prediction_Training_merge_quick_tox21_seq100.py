...
from models.bert_molecule_prediction_tox21 import *
...
class Trainer:
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        total_loss = [0.0 for i in range(12)]
        # 存储所有预测的结果和标记, 用来计算auc
        all_predictions = [[] for _ in range(12)]
        all_labels = [[] for _ in range(12)]
        all_loss = [[] for _ in range(12)]
        ...
        for i, data in data_iter:
            ...
            label = []
            for i in range(12):
                pred.append(predictions[i].detach().cpu().numpy().reshape(-1))
                label.append(data["label"][:,i].cpu().numpy().reshape(-1))
                # print(pred[i].shape)
                # print(label[i].shape)
...
def main():
    ...
    for lr in lr_list:
        ...
        if config["isRegression"]:
            all_lr_loss.append(sum(six_loss) / len(six_auc))
            all_lr_max_loss.append(max(six_loss))
            all_lr_min_loss.append(min(six_loss))
            # 求方差
            average_loss = sum(six_loss) / len(six_auc)
            sum_loss = 0.0
            for i in six_loss:
                sum_loss += (i - average_loss) * (i - average_loss)
            all_lr_variance.append(sum_loss / len(six_auc))
        else:
            all_lr_auc.append(sum(six_auc) / len(six_auc))
            all_lr_max_auc.append(max(six_auc))
            all_lr_min_auc.append(min(six_auc))
            # 求方差
            average_auc = sum(six_auc) / len(six_auc)
            sum_auc = 0.0
            for i in six_auc:
                sum_auc += (i - average_auc) * (i - average_auc)
            all_lr_variance.append(sum_auc / len(six_auc))

    if config["isRegression"]:
        print('best loss : ', max(all_lr_min_loss))
        with open(config["state_dict_dir"] + "/test_output_quick_mmm_r0_r1_5fold_" + str(args.dr) + "_" + \
