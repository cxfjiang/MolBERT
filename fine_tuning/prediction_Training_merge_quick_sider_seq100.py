...
from models.bert_molecule_prediction_sider import *
...
class Trainer:
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        total_loss = [0.0 for i in range(27)]
        # 存储所有预测的结果和标记, 用来计算auc
        all_predictions = [[] for _ in range(27)]
        all_labels = [[] for _ in range(27)]
        all_loss = [[] for _ in range(27)]
        ...
        for i, data in data_iter:
            ...
            if train == "train":
                ...
                # # loss_ = loss[0]
                # # for i in range(1,27):
                # #     loss_ = torch.add(input=loss_, other= loss[i],)
                # loss_ = torch.mean(torch.tensor(loss))
                # # loss_ = loss_ / 27
                # # requires_grad = True
                # loss_.backword()
                # self.optimizer.step()
...
def main():
    ...
    elif args.dataset_name == "tox21" or args.dataset_name == "sider":
        lr_list = [5e-4, 2e-5,5e-6]
    else:
        if args.dr == 1.0:
            lr_list = [1e-4, 2e-4, 5e-4, 5e-5, 2e-5, 1e-5, 5e-6, 1e-6, 5e-7]
        else:
    ...
    for lr in lr_list:
        ...
        for iter in range(l):
            ...
            if config["isRegression"]:
                ...
                six_auc.append(trainer.test(epoch))
    ...
    if config["isRegression"]:
        ...
        with open(config["state_dict_dir"] + "/test_output_quick_mmm_v2_" + str(args.dr) + "_" + \
