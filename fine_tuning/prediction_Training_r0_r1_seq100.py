...
def main():
    ...
    if args.dataset_name == "hiv" :
        lr_list = [0.0001,2e-4, 2e-05, 5e-5, 5e-06]
        # lr_list = [0.0001]
    elif args.dataset_name == "tox21" or args.dataset_name == "sider":
        lr_list = [0.0001,2e-4, 2e-05, 5e-5, 6e-5]
    else:
        if args.dr == 1.0:
           # lr_list = [1e-4, 2e-4, 5e-4, 5e-5, 2e-5, 1e-5, 5e-6, 1e-6]
            lr_list =[1e-4, 5e-4, 5e-5, 2e-5, 1e-6]
        else:
            lr_list = [5e-2, 2e-2,1e-3, 5e-3, 1e-4, 2e-4, 5e-4, 5e-5, 2e-5, 1e-5, 5e-6, 1e-6, 5e-7]
    ...
    if config["isRegression"]:
        ...
        with open(config["state_dict_dir"] + "/test_output_quick_mmm_final_r0_r1_seq50_" + str(args.dr) + "_" + \
