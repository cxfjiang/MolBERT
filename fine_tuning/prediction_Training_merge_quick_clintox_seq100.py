...
from dataset.dataloader_merge_t1 import CLSDataset
from models.bert_molecule_prediction_clintox import *
...
class Trainer:
    def __init__(self, max_seq_len,
        ...
        self.bert_model = Bert_Smiles_Analysis(config=bertconfig,deviceId=config["cudaId"],
                                              isRegression=config["isRegression"])
        # 将模型发送到计算设备(GPU或CPU)
        ...
        # all_parameters = self.bert_model.state_dict()
        # for i in all_parameters:
        #     print(i)
        #
        # print(len(self.optim_parameters))
        # print(self.optim_parameters.named_parameters())
        # for s in self.optim_parameters:
        #     print(s.named_parameters())
        # print(self.optim_parameters)
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        total_loss = [0.0 for i in range(2)]
        # 存储所有预测的结果和标记, 用来计算auc
        all_predictions = [[] for _ in range(2)]
        all_labels = [[] for _ in range(2)]
        all_loss = [[] for _ in range(2)]
        ...
        for i, data in data_iter:
            ...
            label = []
            for i in range(2):
                pred.append(predictions[i].detach().cpu().numpy().reshape(-1).tolist())
                label.append(data["label"][:,i].cpu().numpy().reshape(-1).tolist())
                # print(pred)
                # print(label)
                all_labels[i].extend(label[i])
                all_predictions[i].extend(pred[i])
                # print(pred[i])
            #auc = 0.6
            # 反向传播
            if train == "train":
        ...
        print(all_labels.shape, all_predictions.shape)
        auc = []
        for i in range(2):
            if np.sum(all_labels[:,i] == 1) > 0 and np.sum(all_labels[:,i] == -1) > 0:
                is_valid = all_labels[:,i] ** 2 > 0
                auc.append(roc_auc_score((all_labels[is_valid, i] + 1) / 2, all_predictions[is_valid, i]))

        for loss_ in total_loss:
            print(loss_ / len(all_labels), end=" ")
        print()
...
def main():
    ...
    # lr_list = [2e-5, 5e-5]
    if args.dataset_name == "hiv" :
        lr_list = [0.0001,2e-4, 2e-05, 5e-5, 5e-06, 5e-7]
    elif args.dataset_name == "tox21" or args.dataset_name == "sider" or args.dataset_name == "clintox":
        lr_list = [0.0001,2e-4, 2e-05, 5e-5, 6e-5]
    else:
        if args.dr == 1.0:
            lr_list = [1e-3, 1e-4, 2e-4, 5e-4, 5e-5, 2e-5, 1e-5, 5e-6, 1e-6, 5e-7]
        else:
            lr_list = [5e-2, 2e-2,1e-3, 5e-3, 1e-4, 2e-4, 5e-4, 5e-5, 2e-5, 1e-5, 5e-6, 1e-6, 5e-7]
        # lr_list = [0.0002, 2e-5, 5e-5,]
    lr = [0.0001, 2e-4, 2e-5,5e-5, 6e-5]
    config = {}
    ...
    config["state_dict_dir"] = "../finetu_output/" + args.dataset_name + "/small_hidden_" + str(args.hidden_size) + \
                               "_layers_" + str(args.num_hidden_layers) + "_heads_" + str(args.num_attention_heads)
    ...
    for lr in lr_list:
        ...
        for iter in range(l):
            ...
            if config["isRegression"]:
                six_loss.append(math.sqrt(trainer.test(epoch)))
    ...
    if config["isRegression"]:
        ...
        with open(config["state_dict_dir"] + "/test_output_quick_mmm_" + str(args.dr) + "_" + \
